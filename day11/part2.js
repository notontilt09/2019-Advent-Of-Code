...
// start at top left and move to the right, then down a row and repeat until all of the grid is painted
for (let a = 199; a >= 0; a--) {
  for (let b = 0; b <= 199; b++) {
    let coords = `${b}, ${a}`;
    let visited = panelMap[coords].painted
    if (!visited) {
      ret += ' '
    } else {
      if (panelMap[coords].color === 'black') {
        ret += ' '
      } else {
        ret += '#'
      }
